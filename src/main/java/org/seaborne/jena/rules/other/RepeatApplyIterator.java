...
package org.seaborne.jena.rules.base;
...
/** Repeat Apply == flatMap. */
public class RepeatApplyIterator<T> implements Iterator<T>, Closeable {
    private Iterator<T>              input;
    private boolean                  finished     = false;
    private Iterator<T>              currentStage = null;
    private Function<T, Iterator<T>> stager;

    protected RepeatApplyIterator(Iterator<T> input, Function<T, Iterator<T>> stager) {
        this.input = input;
        this.stager = stager;
    }
    ...
    public boolean hasNext() {
        ...
        for ( ;; ) {
            if ( currentStage == null && input.hasNext() ) {
                ...
                currentStage = stager.apply(nextItem);
            }
